...
const StakeholderSection = ({ intro }) => {
  ...
  return (
    <>
      <div className={`mx-auto px-6 my-10 text-center`}>
        <h2 className="sm:h2 h4">
          Why <span className="text-primary">build</span> when you can{" "}
          <span className="btn rounded bg-primary p-1 px-2 font-medium text-white sm:rounded-lg">
            buy?
          </span>
        </h2>
        <p className={`text-base text-[#878B9E] sm:text-[20px]`}>
      ...
      <div className="relative w-full text-left">
        ...
        >
          {intro.list.map((item, index) => (
            <SwiperSlide
              key={"feature-" + index}
              className="justify-left mb-10 flex items-start"
            >
              <div
              ...
              >
                ...
                <p className="mb-5 h-20 text-sm text-[#878B9E] sm:text-base">
                  {item.subtitle}
                </p>
