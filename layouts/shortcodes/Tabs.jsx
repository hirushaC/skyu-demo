...
function Tabs({ children }) {
  ...
  return (
    <div className="relative ml-5">
      <ul className="mb-0 flex list-none justify-center items-center rounded-lg space-x-4 px-2 bg-[#11172C] py-3">
        {children.map((item, index) => (
          <li
            key={index}
            className={`m-0 cursor-pointer rounded-lg font-medium text-[#E2E4EB] p-2 ${
              index === 0 && "active-tab"
            }`}
      ...
      <ul
        className="mt-1 mb-0 list-none rounded-xl border p-6"
        ref={tabItemsRef}
      >
        {children}
      </ul>
