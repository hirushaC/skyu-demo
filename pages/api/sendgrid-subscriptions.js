...
sendgrid.setApiKey(process.env.SENDGRID_SUBS_API_KEY);

async function sendSubscribeEmail(req, res) {
  try {
    await sendgrid.send({
      to: `${req.body.email}`,
      from: "hirusha.c@insighture.com",
      subject: `Thank you for subscribing to SkyU newsletter`,
      html: `<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
      <html lang="en">
      <head>
        <meta charset="utf-8">
      
        <title>SkyU Newsletter by Insighture</title>
        <meta name="description" content="The SkyU Newsletter">
  ...
  return res.status(200).json({ success: "You have subscribed successfully" });
}
